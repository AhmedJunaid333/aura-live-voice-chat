...
let siteConfiguration = {
  title: "Auralive Enterprise Live Streaming & Audio Broadcast Platform",
  description: "High-performance live streaming, 10/15/20-seat audio broadcast, and VIP social economy platform",
  language: "en"
}
try {
  if (fs.existsSync(path.resolve(__dirname, './.figma/make/site.json'))) {
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      watch: { ignored: ['**/Prototype/**', '**/New-Live-App/**', '**/Screens new/**', '**/.figma/**'] },
    },
    preview: {
      host: '0.0.0.0',
      port: parseInt(process.env.PORT || '8443'),
